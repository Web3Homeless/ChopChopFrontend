...
export function calcOweIsOwed(debts: Debt[], userAddress: string) {
  const userOwe = debts.filter((x) => x.to == userAddress).map(x => x.debt).reduce((x, y) => x + y, 0);
  const userIsOwed = debts.filter((x) => x.from == userAddress).map(x => x.debt).reduce((x, y) => x + y, 0);
